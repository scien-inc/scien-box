...
CENTRAL_SERVER = os.environ.get('CENTRAL_SERVER', 'http://192.168.1.100:5000')
...
def registration_thread():
    while True:
        try:
            ...
            # 中央サーバーに登録
            response = requests.post(f"{CENTRAL_SERVER}/api/register", json=node_info)
            if response.status_code == 200:
                logger.info(f"中央サーバーへの登録に成功しました: {response.json()}")
            else:
                logger.warning(f"中央サーバーへの登録に失敗しました: {response.status_code}")
